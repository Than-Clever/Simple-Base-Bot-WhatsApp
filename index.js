...
const start = async() => {
    ...
    if (global.connect && !client.authState.creds.registered) {
        ...
        await console.clear()
        let customCode = "THANBAIL"
        let pairCode = await client.requestPairingCode(togel.trim(), customCode);
        await sleep(2000)
        await console.log(color(`[ # ] enter that code into WhatsApp : ${pairCode}`, randomcolor));
    };
    ...
    client.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'open') {
            client.newsletterFollow("120363400738305381@newsletter");
            client.newsletterFollow("120363422701490267@newsletter");
            client.newsletterFollow("120363399930956286@newsletter");
